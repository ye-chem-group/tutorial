...
if __name__ == '__main__':
    try:
        fchk = sys.argv[1]
        frag_atmlist_range = sys.argv[2]
        thresh_occ = float(sys.argv[3])
        thresh_vir = float(sys.argv[4])
        mem = float(sys.argv[5]) * 950
        fcderi = sys.argv[6]
        fovL = sys.argv[7]
    except:
        log.warn('Usage: fchk, frag_atmlist_range (e.g., "0,5" means 0,1,2,3,4 the first 5 atoms), thresh_occ, thresh_vir, mem_in_GB, fcderi, fovL')
        sys.exit(1)

    frag_atmlist_range = list(map(int, frag_atmlist_range.split(',')))
    frag_atmlist = [[i] for i in range(*frag_atmlist_range)]

    log.info('Command line arguments')
    log.info('fchk = %s', fchk)
    log.info('frag_atmlist_range = %s', frag_atmlist_range)
    ...
    from lno.minao import minao_nocore
    s1e = mf.get_ovlp()
    iao_coeff = lo.iao.iao(scell, occ_coeff, minao=minao_nocore)
    lo_coeff = lo.orth.vec_lowdin(iao_coeff, s1e)

    moliao = lo.iao.reference_mol(scell, minao=minao_nocore)
    frag_lolist = autofrag_iao(moliao, frag_type='atom', frag_atmlist=frag_atmlist)
